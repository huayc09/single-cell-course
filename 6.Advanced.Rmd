...
4. Label Transfer: singleCellNet
...
Next, we need to create a conda environment with all the required Python packages. `SeuratExtend` provides a function to do this automatically:
...
```{r scvelo_plot, message=FALSE, warning=FALSE, eval=FALSE}
...
![Basic scVelo Plot](scvelo_basic.png)
![Customized scVelo Plot](scvelo_custom.png)
...
- **Entropy**: This represents the uncertainty in cell fate. Higher entropy (darker color) suggests that a cell has multiple potential fates.
...
```{r visualize_cellchat, fig.width=12, fig.height=8, message=FALSE, warning=FALSE}
...
```{r ligand_activity, fig.width = 12, fig.height = 8}
