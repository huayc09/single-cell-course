...
Let's start by creating a basic UMAP plot using ggplot2 instead of Seurat's built-in functions:
...
1. We use `FetchData()` to extract the UMAP coordinates, cluster information, and CD3D expression from our Seurat object.
2. `ggplot()` initializes the plot with our data and aesthetic mappings.
3. `geom_point()` adds a layer of points to represent our cells.
4. `scale_color_viridis()` applies a continuous color scale.
5. `theme_minimal()` applies a minimalistic theme.
6. `labs()` adds labels to our plot.
